...
import AOPTargetsUpload from './components/AOPTargetsUpload';
import AOPTargetsTable from './components/AOPTargetsTable';
import { getAOPTargets, updateAOPTarget } from './services/aopApi';
...
export default function App() {
  ...
  // SIP Targets-specific state
  const [aopTargets, setAopTargets] = useState([]);
  const [aopLoading, setAopLoading] = useState(false);
  const [aopError, setAopError] = useState('');

  // Updated color palette for charts to match app style
  const CHART_COLORS = [
    '#57c9a0', 
    '#203a43',
    '#3aa181', 
  ...
  const pieChartData = useMemo(() => {
    ...
    const regionalPayouts = allTableData.reduce((acc, row) => {
      ...
      const amount =
        parseFloat(String(row['SIP Payout Amount']).replace(/[^0-9.-]/g, '')) ||
        0;
  ...
  // Prepare data for bar chart - top 10 employees by SIP Payout
  const topEmployeesData = useMemo(() => {
    if (!allTableData.length) return [];
    const employeePayouts = allTableData.reduce((acc, row) => {
      const empId = row['Emp ID'] || 'Unknown';
      const amount =
        parseFloat(String(row['SIP Payout Amount']).replace(/[^0-9.-]/g, '')) ||
        0;
      acc[empId] = (acc[empId] || 0) + amount;
      return acc;
    }, {});
    return Object.entries(employeePayouts)
      .map(([name, value]) => ({ name, value }))
      .sort((a, b) => b.value - a.value)
      .slice(0, 10);
  }, [allTableData]);
  ...
  const fetchAOPTargets = async () => {
    setAopLoading(true);
    setAopError('');
    try {
      const response = await getAOPTargets();
      setAopTargets(response.data);
    } catch (err) {
      setAopError(err.response?.data?.error || 'Failed to load SIP Targets');
    } finally {
      setAopLoading(false);
    }
  };
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      if (activeTab === 'statements') {
        ...
        setTotals(allTotals);
      } else if (activeTab === 'aopTargets') {
        await fetchAOPTargets();
      } else {
    ...
    <div className={`app-container ${sidebarOpen ? 'sidebar-open' : ''}`}>
      <nav className="glass-nav modern-background">
        <div className="nav-brand">
          <h1>
            SIP <span className="gradient-text">CASS</span>
          </h1>
      ...
      <div className="app-grid">
        <aside
          className="glass-sidebar modern-background"
          style={{
            display: 'flex',
        ...
        >
          <div className="sidebar-tabs">
            ...
            <button
              className={`sidebar-tab ${activeTab === 'aopTargets' ? 'active' : ''}`}
              onClick={() => {
                setActiveTab('aopTargets');
                setSidebarOpen(false);
              }}
            >
              <span className="tab-icon">
              ...
              <span className="tab-label">SIP Targets</span>
              {activeTab === 'aopTargets' && <div className="active-indicator" />}
            </button>
          </div>
        ...
        <main className="glass-main">
          ...
          {error && (
            <div className="error-glass">
              ...
              <button
                onClick={() => setError('')}
                className="glass-button small close-button"
              >
                <svg
                  width="16"
                  height="16"
          ...
          {!loading && !error && (
            <>
                  ...
                  <div className="metric-cards">
                    <div className="luxury-card">
                      ...
                      <p className="metric-value">
                        ${summary.totalSipPayout?.toLocaleString() || '0'}
                      </p>
                    </div>
                    ...
                    <div className="luxury-card">
                      ...
                      <p className="metric-value">
                        {summary.paymentSuccessRate || '0'}%
                      </p>
                    </div>
                  </div>

                  <div
                    className="chart-section luxury-card"
                    style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'stretch' }}
                  >
                    {/* Pie Chart */}
                    <div style={{ flex: 1, minWidth: 300 }}>
                      {pieChartData.length > 0 ? (
                        ...
                        <p className="no-chart-data">
                          No regional payout data available for the chart.
                        </p>
                      )}
                    </div>
                    ...
                    <div style={{ flex: 1, minWidth: 300 }}>
                      {topEmployeesData.length > 0 ? (
                        ...
                        <p className="no-chart-data">
                          No employee payout data available for the chart.
                        </p>
                      )}
                    </div>
                  </div>
              ...
              {activeTab === 'upload' && user?.position !== 'Seller' && (
                <div className="upload-view">
                  <h2 className="upload-section-title">
                    Upload your SIP Payout Calculation file here
                  </h2>
                  <FileUpload onUploadSuccess={handleUploadSuccess} />
                </div>
              )}

              {activeTab === 'aopTargets' && (
                <div className="aop-targets-view">
                  {user?.position !== 'Seller' && (
                    <div
                    ...
                    >
                      <AOPTargetsUpload onUploadSuccess={fetchAOPTargets} />
                    </div>
                  )}
                  {aopError && <div className="error">{aopError}</div>}
                  {aopLoading ? (
                    <p>Loading SIP Targets...</p>
                  ) : (
                    <AOPTargetsTable
                      data={aopTargets}
                      onRowSave={async (id, updatedData) => {
                        try {
                          await updateAOPTarget(id, updatedData);
                          fetchAOPTargets();
                        } catch {
                          alert('Failed to save row. Try again.');
                        }
                      }}
                    />
                  )}
